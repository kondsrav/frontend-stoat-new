...
export class Client extends AsyncEventEmitter<Events> {
  ...
  constructor(options?: Partial<ClientOptions>, configuration?: RevoltConfig) {
    ...
    // Try to restore session early so any initial API calls include authentication
    try {
      const stored = typeof localStorage !== "undefined" ? localStorage.getItem("revolt_session") : null;
      if (stored) {
        try {
          const parsed = JSON.parse(stored);
          console.debug("revolt.js: restoring session from localStorage");
          this.useExistingSession(parsed as Session);
          // establish connection using restored session
          this.connect();
        } catch (e) {
          console.debug("revolt.js: restoring session token from localStorage");
          this.useExistingSession(stored as Session);
          this.connect();
        }
      }
    } catch (e) {
  ...
  async login(details: DataLogin): Promise<void> {
    ...
    if (data.result === "Success") {
      this.#session = data;
      // Persist session to localStorage to survive hot-reloads / refreshes in dev
      try {
        if (typeof localStorage !== "undefined") {
          localStorage.setItem("revolt_session", JSON.stringify(data));
        }
      } catch (e) {
        // ignore
      }
      // Update the API headers to include the new session and connect the event client
      this.#updateHeaders();
      // Establish websocket connection using the newly-acquired session
      this.connect();
    } else {
