...
export function AddMembersToGroupModal(props: DialogProps & Modals & { type: "add_members_to_group" }) {
  ...
  // Force empty filter with multiple safeguards
  ...
  // Multiple ways to ensure filter starts empty
  setFilter(""); // Clear immediately
 
  // Force clear after component mounts
  setTimeout(() => setFilter(""), 0);
  setTimeout(() => setFilter(""), 100);
 
  // Clear any potential garbage values when modal opens
  console.log("🚨 FILTER DEBUG - Initial filter value:", filter());
  console.log("🚨 FILTER DEBUG - Forcing empty string");
  setFilter("");
  console.log("🚨 FILTER DEBUG - After clear:", filter());
    ...
    <Dialog minWidth={420} show={props.show} onClose={props.onClose} title={<Trans>Add member to group</Trans>} actions={[{ text: <Trans>Close</Trans> }]}>
      <Column gap="lg">
        <TextField
          value={""}
          variant="filled"
          placeholder="Type 2-3 letters to search all users..."
          onInput={(e) => {
            const value = e.currentTarget.value;
            console.log("🚨 INPUT DEBUG - Raw value:", value);
            ...
            console.log("🚨 INPUT DEBUG - Sanitized value:", sanitized);
            setFilter(sanitized);
            console.log("🚨 INPUT DEBUG - Filter state after set:", filter());
