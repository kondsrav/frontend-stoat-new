...
interface Props {
   ...
   * Text replacement
   */
  onTextReplacement: (node: PMNode) => void;
}
...
function Picker(
  ...
  function onMouseDown() {
    // Only close if clicking outside the picker element. We use the
    // floating element (setFloating) as the container to check against.
    return;
  }
  ...
  onMount(() => {
    const handler = (ev: MouseEvent) => {
      ...
      // If click happened inside the floating picker element or inside the
      // trigger anchor, don't close. Otherwise hide picker.
      if (target && (node.contains(target) || (anchor && anchor.contains(target)))) {
        return;
      }
