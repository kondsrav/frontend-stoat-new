...
export function FileInfo(props: Props) {
  return (
    <Base align>
      ...
      <Show
        when={
          props.file?.downloadURL || props.file?.originalUrl || props.file?.url
        }
      >
        {
          /* Prefer explicit downloadURL, then originalUrl, then preview url */
        }
        <Action
          href={
            props.file?.downloadURL || props.file?.originalUrl || props.file?.url
          }
          // Hint the browser to download with the original filename when possible
