...
try {
    ...
    if ($frontendHealth.StatusCode -eq 200) {
        Write-Host "[SUCCESS] Frontend is running and healthy" -ForegroundColor Green
        Write-Host "   URL: http://localhost:3000" -ForegroundColor White
        Write-Host "   Status: $($frontendHealth.StatusCode) $($frontendHealth.StatusDescription)" -ForegroundColor White
    }
} catch {
    Write-Host "[ERROR] Frontend is not accessible: $($_.Exception.Message)" -ForegroundColor Red
}
...
foreach ($port in $backendPorts.Keys) {
    ...
    try {
        if ($port -eq "14703") {
            ...
            Write-Host "[INFO] Port $port ($service): WebSocket endpoint (cannot test with HTTP)" -ForegroundColor Yellow
        } else {
            $response = Invoke-WebRequest -Uri "http://localhost:$port" -UseBasicParsing -TimeoutSec 5
            Write-Host "[SUCCESS] Port $port ($service): $($response.StatusCode) $($response.StatusDescription)" -ForegroundColor Green
        }
    } catch {
        Write-Host "[ERROR] Port $port ($service): Not accessible" -ForegroundColor Red
    }
}

# Test Frontend to Backend API connectivity
Write-Host "`nTesting Frontend to Backend Connection..." -ForegroundColor Yellow

# Check if frontend can reach backend through Docker networking
$dockerContainers = docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
Write-Host "`nDocker Containers Status:" -ForegroundColor Cyan
Write-Host $dockerContainers -ForegroundColor White

# Test connection from inside the frontend container
Write-Host "`nTesting API connectivity from frontend container..." -ForegroundColor Yellow
try {
    ...
    if ($testConnection -like "*SUCCESS*") {
        Write-Host "[SUCCESS] Frontend container can reach backend API" -ForegroundColor Green
    } else {
        Write-Host "[ERROR] Frontend container cannot reach backend API" -ForegroundColor Red
        Write-Host "   Make sure host.docker.internal is working in your Docker setup" -ForegroundColor Yellow
    }
} catch {
    Write-Host "[ERROR] Could not test container connectivity: $($_.Exception.Message)" -ForegroundColor Red
}
...
Write-Host "`nTo stop the containers:" -ForegroundColor Cyan
Write-Host "   docker stop stoat-frontend-test" -ForegroundColor White
Write-Host "   docker rm stoat-frontend-test" -ForegroundColor White
